...
|`text\_to\_image\_value`|当内容类型是 `mode\_switch\_\*` 时显示，填写 `/生图` 使用的值|
|`image\_to\_image\_value`|当内容类型是 `mode\_switch\_\*` 时显示，填写 `/改图` 使用的值|
...
|`mode\_switch\_text`|同一字段在 `/生图` 与 `/改图` 下使用不同文本值|
|`mode\_switch\_number`|同一字段在 `/生图` 与 `/改图` 下使用不同数值|
|`mode\_switch\_json`|同一字段在 `/生图` 与 `/改图` 下使用不同 JSON 值，常用于切换节点引用|
