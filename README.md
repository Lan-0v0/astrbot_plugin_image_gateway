...
AstrBot 多模型图像生成网关插件。支持 OpenAI、Gemini、阿里云百炼、火山引擎、MiniMax、智谱、腾讯混元等图像 API，以及 ComfyUI / A1111 Stable Diffusion WebUI 工作流。可自定义 baseURL、模型名称、工作流 ID、优先级、重试次数及审核力度，支持文生图、图生图与自然语言触发。

## 功能特性

* **多模型网关**：可同时配置多个国内外图像模型，按优先级依次尝试
* **工作流（Workflow）支持**：可配置 ComfyUI 与 A1111 WebUI 工作流，与模型统一参与优先级调度，并支持单工作流双入口
* **文生图 / 改图**：支持 `/生图` 与 `/改图` 指令
* **条目专属指令**：模型与 Workflow 可退出默认调度，改由用户配置的独立指令精确调用
...
* **审核力度可调**：OpenAI / Gemini 均支持将审核设为 `none`，按失败分类穷尽参数链与备用路由后再判定敏感错误
...
|`enabled`|是否启用|
|`priority`|优先级数值，默认 `1`，数值越大越优先。与工作流条目共用同一套排序；填 `0` 表示随机优先级|
...
|`dedicated\_command`|专属指令，默认留空。填写后该模型不再参与 `/生图`、`/改图`，改由该指令单独调用；填写时无需输入 `/`|
...
|-|-|
|`enabled`|是否启用|
|`priority`|优先级数值，默认 `1`，数值越大越优先。与模型条目共用同一套排序；填 `0` 表示随机优先级|
|`retry\_count`|重试次数，`-1` 表示使用全局默认|
...
|`dedicated\_command`|专属指令，默认留空。填写后该工作流退出 `/生图`、`/改图` 默认调度，改由该指令单独调用|
|`fake\_forward\_mode`|消息伪造转发。默认 `follow\_global`；可选关闭、Bot自身、生图要求者、自定义QQ号|
|`fake\_forward\_custom\_qq`|仅当 `fake\_forward\_mode=custom\_qq` 时显示，填写自定义 QQ 号|
...
> 自 v2.1.9 起，该超时只在 ComfyUI 确实不再处理该任务时才生效。到达超时后插件会查询 `/queue`，只要任务仍在运行或排队就继续等待（60 秒一轮，最多 30 轮），因此一张即将完成的图不会因为超时设置偏小而被丢弃。

### LLM提示词扩展（`llm\_prompt\_expansion`）

位于「ComfyUI 运行环境（全局默认）」与「生图开始提示」之间。关闭时不会调用 LLM 扩展提示词，相关附属配置项也会在配置面板中隐藏。开启后，插件会在真正生图前调用所选 LLM，将用户原始提示词扩展为更详细的图像生成描述。

|配置项|说明|默认值|
|-|-|-|
|`enabled`|是否启用 LLM 提示词扩展|`false`|
|`llm\_provider\_id`|扩展提示词时使用的 LLM 提供商；留空则跟随当前默认配置|`''`|
|`custom\_persona\_prompt`|作为 LLM 扩展时的系统人设提示词；留空则使用内置默认人设|`你是一位专业的图像生成提示词工程师，擅长将简短描述扩展为细节丰富、适合 Stable Diffusion / ComfyUI 的高质量提示词。`|

说明：

* 扩展失败时会自动回退原始提示词，不影响既有生图/改图链路
* 该功能与「生图开始提示」中的 LLM 模式相互独立，可分别配置不同的提供商与人设
...
* 设为 **`none`** 时，插件会先尝试：不传 `moderation` 参数 → `low` → `auto`；若遇内容拦截，会跳过同质参数重试并继续走 `/chat/completions` 备用路由。整条链穷尽且曾出现内容拦截时，才返回敏感内容错误。
* 设为 **`high`** 时，会以更严格的审核等级请求上游接口。

**Gemini** 可选：`none` / `low` / `high` / `default`

* 设为 **`none`** 时，插件会依次尝试 `BLOCK\_NONE`、`OFF`、`BLOCK\_ONLY\_HIGH`、省略 `safetySettings`、非对称 safety 等多种设置；失败后继续 OpenAI 兼容图像路由。中间步骤的内容拦截**不会**提前终止整条链。
...
* **数值越大越优先**，例如 `10` 会排在 `3` 前面
* **模型与工作流共用同一套排序**，所以优先级 `10` 的工作流一定排在优先级 `3` 的 API 模型前面
* **填 `0` 表示随机优先级**：所有填 `0` 的条目会排在有优先级的条目之后，并在每次请求时随机排序
* **同优先级保持配置顺序**，方便你在面板里直观控制并列条目

常见用法：

* 想固定主力 + 备用：主力填较大的数（如 `10`），备用依次填 `5`、`3`、`1`
* 想在几个条目之间随机挑选：把它们**都**填 `0`
* 想“主力优先，失败后随机回退”：主力填 `10`，其余全部填 `0`

预留间隔（如 `10` / `20` / `30`）会更好插入新条目，但不是必须的，任何整数都可以。

> \*\*从 v2.1.7 及更早版本升级\*\*：旧的「优先级」预设单选框已移除。插件首次加载时会自动把旧配置迁移成纯数值（最高=`40`、高=`30`、普通=`20`、低=`10`、最低=`1`），并保留你在旧面板数值框里手填过的非默认值，无需手动改配置。
...
模型或 Workflow 的“专属指令”留空时，行为与旧版本完全一致，继续按优先级参与 `/生图` 和 `/改图`。填写后，该条目退出默认调度，只响应自己的指令。
...
* 填写时不需要输入 `/`
* 不附带图片时执行文生图，末尾可加生成张数
* 附带图片或引用含图片的消息时自动执行改图
* 专属指令只调用对应条目，不会回退到其它模型或工作流
* 多个已启用条目不能配置相同的专属指令；重复时插件会返回明确提示
* `生图`、`改图` 为保留指令，不能作为专属指令
...
1. 若启用了 LLM 提示词扩展，真正生图前会先用所选 LLM 扩展用户提示词；扩展失败时自动回退原始提示词
2. 若启用了开始提示功能，生成开始时会先发送一条开始提示：固定语句模式下随机发送一条预设文案；LLM 模式下由所选提供商根据人设生成
3. 生成成功后：
...
* 工作流超时是「软超时」：超时后若 ComfyUI 队列中仍有该任务，插件会继续等待直到任务完成或离开队列
...
* v2.2.2 里 `/tag` 有时只回复「正在识别图片标签…」就没了下文，且**日志没有任何报错**——原因是原实现连续 `yield` 两条消息，而其他插件（日志中表现为 `xxx - _after_message_sent_impl stopped event propagation`）在第一条消息发出后停止了事件传播，后续 `yield` 不再被取用
* v2.2.3 改为与 `/生图`、`/改图` 一致的**主动发送**：进度提示和标签结果都直接发送，只有主动发送失败才退化为单条 `yield`；即使事件传播被中途停止，标签也一定发得出去
* 识别失败、图片解码失败的提示同样走主动发送
...
### 配置面板条目介绍修复（工作流列表 / 工作流自定义节点条目）

* 此前版本依赖给 AstrBot Dashboard 打补丁才能显示条目介绍，AstrBot 升级后前端产物被官方覆盖，介绍随之消失。v2.2.0 改为「官方兼容」写法，任何版本的 AstrBot 都能显示条目介绍，不再空白：
  * 「工作流（Workflow）列表」每个条目的介绍来自「工作流 ID (workflow_id)」中的变量，显示为 `工作流 ID: miaomiao文生图`；
  * 「工作流自定义节点条目」每个条目的介绍由插件自动生成并持久化到隐藏字段 `display_summary`，内容为「显示名称——所属工作流 ID」，显示为 `介绍: 正向提示词——miaomiao文生图`；
  * 「显示名称」「所属工作流 ID」任一端留空时自动退化为另一端；修改后保存配置即可（插件热重载时自动刷新介绍）。
* 同时在模板元数据中新增 `display_item_list`：如果给 AstrBot Dashboard 应用了「条目介绍补丁」（见 `D:\Mine\Plugins\需求与产出\Dashboard条目介绍补丁`），介绍会进一步升级为**纯值显示**——工作流条目只显示 `miaomiao文生图`，节点条目只显示 `正向提示词——miaomiao文生图`；未打补丁时自动回退到上述官方形式，两种形态互不冲突。
* 启动时会自动清理旧版遗留的工作流条目 `display_name`、`display_summary` 字段，并按最新规则重建节点条目的 `display_summary`，无需手动迁移。
* **重要**：更新插件后请重载插件或重启 AstrBot，让归一化逻辑执行一次并刷新介绍文案。

## v1.5.0 补充说明

### LLM提示词扩展

* 配置面板在「ComfyUI 运行环境（全局默认）」与「生图开始提示」之间新增「LLM提示词扩展」区块
* 关闭开关时，「LLM 提供商」与「自定义人设提示词」会自动隐藏；开启后可选择 AstrBot 中已配置的模型提供商，并填写自定义人设提示词
* 开启后，插件会在真正生图前调用 LLM 扩展用户提示词；扩展失败时自动回退原始提示词，不影响其他功能
* **重要**：修改 schema 后需要重启 AstrBot 或重载插件才能生效

## v1.4.8 补充说明

### 工作流条目副标题硬化修复

* ComfyUI 工作流模板现在保留 `display_item: ["display_summary"]`，并加回 `hide_hint_in_list: true`，避免已有工作流卡片继续把模板介绍文案显示在副标题区域
* 插件启动归一化配置时，会自动清理工作流条目中的旧版遗留字段 `display_name`，防止旧配置里的 `miaomiao文生图` 被旧前端或旧 schema 当作固定前缀继续拼到介绍文案前面
* 最终效果：在“工作流 ID (workflow_id)”里填写 `文生图1` 时，已有工作流条目副标题只显示 `文生图1`
* **重要**：升级后请重载插件或重启 AstrBot，让插件归一化逻辑执行一次并清理旧字段
