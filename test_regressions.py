...
from astrbot_plugin_image_gateway.adapters.hunyuan import HunyuanAdapter  # noqa: E402
...
class WorkflowConfigRegressionTests(unittest.TestCase):
    def test_conf_schema_uses_provider_names_for_model_templates(self) -> None:
        ...
        self.assertEqual(schema["models"]["templates"]["dashscope"]["name"], "阿里云百炼")
        self.assertEqual(schema["models"]["templates"]["volcengine"]["name"], "火山引擎")
        self.assertEqual(schema["models"]["templates"]["minimax"]["name"], "MiniMax")
        self.assertEqual(schema["models"]["templates"]["zhipu"]["name"], "智谱")
        self.assertEqual(schema["models"]["templates"]["hunyuan"]["name"], "腾讯混元")

    def test_conf_schema_exposes_a1111_workflow_template(self) -> None:
        schema = json.loads((repository_root / "_conf_schema.json").read_text(encoding="utf-8"))
    ...
    def test_conf_schema_describes_workflow_binding_as_dual_locator_rules(self) -> None:
        ...
        self.assertIn("节点ID+字段路径", bindings_section["hint"])
        self.assertIn("_root", bindings_section["hint"])
    ...
    def helper_from_template_entry_parses_bindings_and_defaults(self) -> None:
        ...
        self.assertEqual(workflow_config.workflow_engine, "comfyui")
    ...
    def test_from_template_entry_reads_a1111_workflow_engine(self) -> None:
        workflow_config = WorkflowConfig.from_template_entry(
            {
                "__template_key": "a1111",
                "workflow_id": "local-sd",
                "workflow_content": json.dumps({"prompt": "", "steps": 20}),
            }
        )
        self.assertEqual(workflow_config.workflow_engine, "a1111")
...
class ProviderAdapterRegressionTests(unittest.TestCase):
    def test_get_adapter_supports_new_providers(self) -> None:
        ...
        self.assertIsInstance(get_adapter("volcengine"), VolcengineAdapter)
        self.assertIsInstance(get_adapter("ark"), VolcengineAdapter)
        self.assertIsInstance(get_adapter("minimax"), MiniMaxAdapter)
        self.assertIsInstance(get_adapter("zhipu"), ZhipuAdapter)
        self.assertIsInstance(get_adapter("hunyuan"), HunyuanAdapter)
        self.assertIsInstance(get_adapter("tencent_hunyuan"), HunyuanAdapter)

    def test_get_adapter_rejects_unknown_provider(self) -> None:
        with self.assertRaises(GenerationError):
            get_adapter("unknown-provider")

    def test_dashscope_normalizes_size_and_base_url(self) -> None:
        self.assertEqual(DashScopeAdapter._normalize_size("1024x1024"), "1024*1024")
        self.assertEqual(
            DashScopeAdapter._normalize_base_url("https://dashscope.aliyuncs.com"),
            "https://dashscope.aliyuncs.com/api/v1",
        )

    def test_zhipu_normalizes_base_url(self) -> None:
        self.assertEqual(
            ZhipuAdapter._normalize_base_url("https://open.bigmodel.cn/api/paas"),
            "https://open.bigmodel.cn/api/paas/v4",
        )

    def test_model_config_from_template_entry_reads_dashscope_template_key(self) -> None:
        model_config = ModelConfig.from_template_entry(
            {
                "__template_key": "dashscope",
                "display_name": "百炼测试",
                "model_name": "wan2.6-t2i",
                "negative_prompt": "低质量",
            }
        )
        self.assertEqual(model_config.provider, "dashscope")
        self.assertEqual(model_config.raw.get("negative_prompt"), "低质量")


class ProviderAdapterAsyncRegressionTests(unittest.IsolatedAsyncioTestCase):
    class FakeResponse:
    ...
    def build_model(self, provider: str, **overrides: Any) -> ModelConfig:
        defaults = {
            "provider": provider,
    ...
    async def test_minimax_text_to_image_parses_url_response(self) -> None:
        ...
        with patch(
            "astrbot_plugin_image_gateway.adapters.minimax.download_image",
            return_value=Path("minimax.png"),
        ):
            paths = await adapter.text_to_image("cat", 1, model, Path("."), session)

        self.assertEqual(paths, [Path("minimax.png")])
        self.assertTrue(session.post_urls[0].endswith("/v1/image_generation"))

    async def test_dashscope_wan26_sync_text_to_image(self) -> None:
                            ...
                            "choices": [
                                {
                                    "message": {
                                        "content": [
                                            {"image": "https://example.com/wan.png", "type": "image"}
                                        ]
                                    }
                                }
                            ]
        ...
        with patch(
            "astrbot_plugin_image_gateway.adapters.dashscope.download_image",
            return_value=Path("dashscope.png"),
        ):
            paths = await adapter.text_to_image("花店", 1, model, Path("."), session)

        self.assertEqual(paths, [Path("dashscope.png")])
        self.assertIn("/multimodal-generation/generation", session.post_urls[0])


class A1111WorkflowRegressionTests(unittest.IsolatedAsyncioTestCase):
    class FakeResponse:
        def __init__(self, *, status: int = 200, json_data: Any = None):
            self.status = status
            self._json_data = json_data
    ...
    class FakeSession:
        ...
        def post(self, url: str, **kwargs):
            self.posts.append((url, kwargs))
            return A1111WorkflowRegressionTests.FakeResponse(
                json_data={"images": ["aGVsbG8="]}
            )

    def build_workflow_config(self) -> WorkflowConfig:
        return WorkflowConfig.from_template_entry(
            {
                "__template_key": "a1111",
                "workflow_id": "a1111-demo",
    ...
    async def test_generate_text_to_image_posts_txt2img_payload(self) -> None:
        runner = A1111WorkflowRunner()
        session = self.FakeSession()
        workflow_config = self.build_workflow_config()
        node_bindings = [
            WorkflowNodeBinding(
        ...
        with patch(
            "astrbot_plugin_image_gateway.services.a1111_runner.save_base64_image",
            return_value=Path("a1111.png"),
        ):
            paths = await runner.generate_text_to_image(
                ...
                workflow_config,
                node_bindings,
                WorkflowRuntimeConfig(base_url="http://127.0.0.1:7860"),
                Path("."),
                session,
            )

        self.assertEqual(paths, [Path("a1111.png")])
        self.assertTrue(session.posts[0][0].endswith("/sdapi/v1/txt2img"))
        self.assertEqual(session.posts[0][1]["json"]["prompt"], "sunset")

    async def test_generate_image_to_image_posts_img2img_payload(self) -> None:
        runner = A1111WorkflowRunner()
        session = self.FakeSession()
        workflow_config = self.build_workflow_config()
        ...
        with patch(
            "astrbot_plugin_image_gateway.services.a1111_runner.save_base64_image",
            return_value=Path("a1111_edit.png"),
        ):
            paths = await runner.generate_image_to_image(
                "make it cinematic",
                ["aGVsbG8="],
        ...
        self.assertEqual(paths, [Path("a1111_edit.png")])
        self.assertTrue(session.posts[0][0].endswith("/sdapi/v1/img2img"))
        self.assertEqual(session.posts[0][1]["json"]["init_images"], ["aGVsbG8="])


class WorkflowRootBindingRegressionTests(unittest.TestCase):
    def test_merge_root_binding_overwrites_top_level_prompt(self) -> None:
        workflow_config = WorkflowConfig.from_template_entry(
            {
                "__template_key": "a1111",
                "workflow_id": "a1111-root",
                "workflow_content": json.dumps({"prompt": "placeholder", "steps": 20}),
            }
        )
        node_bindings = [
            WorkflowNodeBinding(
        ...
        self.assertEqual(merged_payload["prompt"], "final prompt")
        self.assertEqual(merged_payload["steps"], 20)


class A1111SchedulingRegressionTests(unittest.IsolatedAsyncioTestCase):
    async def test_generation_service_routes_a1111_workflow_engine(self) -> None:
        workflow = WorkflowConfig.from_template_entry(
            {
                "__template_key": "a1111",
                "workflow_id": "a1111-route",
                "workflow_content": json.dumps({"prompt": "", "steps": 10}),
            }
        )
        service = GenerationService(
            [workflow],
        ...
        async def fake_generate_text_to_image(
            ...
            self.assertEqual(workflow_config.workflow_engine, "a1111")
            return [Path("routed.png")]

        with (
            patch.object(service._a1111_runner, "generate_text_to_image", fake_generate_text_to_image),
            patch(
                "astrbot_plugin_image_gateway.services.generation.aiohttp.ClientSession",
                FakeClientSession,
            ),
        ):
            paths, target_name, _send_strategy, _fake_forward = await service.generate(
                mode="text_to_image",
                prompt="test",
            )
